...
public class AVLView extends Pane {
    private final AVL<Integer> tree;
    private final double vGap = 50;
    private final Map<AVLNode<Integer>, Circle> nodeCircles = new HashMap<>();

    public AVLView(AVL<Integer> tree) {
        this.tree = tree;
     ...
     */
    private void displayTree(AVLNode<Integer> current, double x, double y, double hGap) {
    ...
    public void highlightNode(Integer element) {
        AVLNode<Integer> node = tree.searchNode(element);
        if (node != null) {
